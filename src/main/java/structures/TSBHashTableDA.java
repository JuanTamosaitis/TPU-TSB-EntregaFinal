 ...
 *
 * @author TPU-G21 Cabral - García - Sánchez - Tamosaitis
...
public class TSBHashTableDA<K,V> implements Map<K,V>, Cloneable, Serializable
{
    ...
    //la cantidad de objetos incluida la cantidad de tumbas..
    ...
    public boolean containsKey(Object key) 
    {
        ...
        int x = this.search_for_index((K)key, ik);

        if (x == -1) { return false; }

        Entry<K,V> entry = (Entry<K, V>) table[x];

        return (entry.getState() != TOMBSTONE);
    }
    ...
    public V remove(Object key)
    {
        ...
        if (x != null){
            ...
            Entry<K,V> entry = new Entry<>(null, null, TOMBSTONE);
            int ib = this.search_for_index((K)key, ik);

            this.table[ib] = entry;

            //se realiza el decremento en el count pero no en el totalCounter (el cual tiene en cuenta las tumbas)
            this.count--;
    ...
    public void clear() 
    {
        ...
        for (int i = 0; i < this.table.length; i++) {
            ...
            this.table[i] = new Entry<>(null, null, OPEN);
    ...
    public int hashCode() 
    {
        ...
        int hash = Objects.hash(initial_capacity, load_factor);
        hash = 31 * hash + Arrays.hashCode(table);
        return hash;
    }
    ...
    public boolean contains(Object value)
    {
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            if (this.table[i] != null){
                ...
                if (value == entry.getValue() && entry.getState() != TOMBSTONE) return true;
            }
    ...
    protected void rehash()
    {
        ...
        //iguala los contadores
        this.totalCounter = this.count;
    }
    ...
    {
        return (float) this.totalCounter / this.table.length;
    } 
    ...
    private class KeySet extends AbstractSet<K> 
    {
        ...
        private class KeySetIterator implements Iterator<K>
        {
            ...
            public boolean hasNext() 
            {
                ...
                if(current >= t.length-1) {return false;}

                next_entry = current + 1;

                Entry<K,V> entry = (Entry<K, V>) t[next_entry];

                if(counter == count) {return false;}

                while(next_entry < t.length && entry.getState() != CLOSED)
                {
            ...
            public K next() 
            {
                ...
                Entry<K,V> entry = (Entry<K, V>) table[current];
                counter++;
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>>
    {
        ...
        public boolean contains(Object o) 
        {
            ...
            if(TSBHashTableDA.this.search_for_entry(key, index) == null) {return false;}
